...
const sanitizeFilename = (name: string, fallback: string = 'untitled_export'): string => {
...
const App: React.FC = () => {
  ...
  const handleExportSelected = () => {
    ...
    const baseFilename = sanitizeFilename(customExportFilename.trim(), 'selected_collection');
    exportConversations(selected, `${baseFilename}.json`);
  };

  const handleExportUnselected = () => {
    const unselected = processedConversations.filter(conv => !selectedConversationIds.has(conv.id));
    const baseFilename = sanitizeFilename(customExportFilename.trim(), 'unselected_conversations');
    exportConversations(unselected, `${baseFilename}.json`);
  };
      ...
      <header className="mb-8">
        ...
        <div className="bg-gray-800 p-4 sm:p-6 rounded-xl shadow-2xl">
          ...
          <div className="mb-4 flex justify-center">
            <input
              type="text"
              placeholder="Custom export filename (e.g., my_archive)"
              value={customExportFilename}
              onChange={(e) => setCustomExportFilename(e.target.value)}
              className="w-full max-w-lg bg-gray-700 border border-gray-600 text-gray-200 placeholder-gray-400 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 shadow-sm"
              aria-label="Custom export filename (.json extension will be added automatically)"
            />
          </div>
...
const ConversationItem: React.FC<ConversationItemProps> = ({ conversation, isSelected, onToggleSelect }) => {
  ...
  const handleExportAsTextFile = () => {
    ...
    // Use the updated sanitizeFilename, providing a specific fallback for this context
    const baseFilename = sanitizeFilename(conversation.title || conversation.id, `conversation_${conversation.id}`);
    const filename = `ChatTown_Obsidian_${baseFilename}.txt`;
